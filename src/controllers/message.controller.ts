...
class MessagesController {
  ...
  sendMessage(id: number, message: string) {
    try {
      ...
      if (socket.readyState === WebSocket.CONNECTING) {
        console.log("WebSocket is still connecting, waiting to send message...");
  ...
  private onMessage(id: number, messages: Message | Message[]) {
    try {
      ...
      // Фильтруем новые сообщения, чтобы не добавлять уже существующие
      const filteredMessagesToAdd = messagesToAdd.filter(newMessage => {
        return !currentMessages.some(existingMessage => existingMessage.time === newMessage.time);
      });
  
      if (filteredMessagesToAdd.length > 0) {
        // Обрабатываем сообщения перед добавлением
        const processedMessages = processMessages([...currentMessages, ...filteredMessagesToAdd] as Message[]);
  
        // Сохраняем сообщения в store для данного чата
        const updatedMessages = store.getState().messages || {};
        updatedMessages[id] = processedMessages;
  
        store.set('messages', updatedMessages);
      }
    } catch (e) {
      console.error('Failed to add messages for chat', e);
    }
  }
