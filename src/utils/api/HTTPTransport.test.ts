...
export class HTTPTransport {
  ...
  private request<R = unknown>(url: string, options: Options, timeout = 5000): Promise<R> {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      const fullUrl = options.data && options.method === METHODS.GET
        ? `${this.baseUrl}${url}?${new URLSearchParams(options.data).toString()}`
        : `${this.baseUrl}${url}`;
